...
public class MatchTests
{
    ...
    public void Match_Failure_InvokesOnFailure()
    {
        ...
        int result = op.Match(
            ...
            {
                Error.Validation v => v.Message.Length,
                Error.NotFound n => n.Message.Length,
                Error.Unauthorized u => u.Message.Length,
                Error.Unexpected u => u.Message.Length,
    ...
    public void Match_ReturnsCorrectType()
    {
        ...
        string result = op.Match(
            ...
            onFailure: err => err switch
            {
                Error.Validation v => $"Error: {v.Message}",
                Error.NotFound n => $"Error: {n.Message}",
                Error.Unauthorized u => $"Error: {u.Message}",
                Error.Unexpected u => $"Error: {u.Message}",
                _ => throw new InvalidOperationException()
            }
        );
