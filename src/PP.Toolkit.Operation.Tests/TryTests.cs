...
public class TryTests
{
    ...
    {
        var op = Operation.Try<int>(() => throw new InvalidOperationException("boom"));
    ...
    {
        var op = await Operation.TryAsync(async () =>
        {
            await Task.Delay(10);
            return "ok";
        });
    ...
    {
        var op = await Operation.TryAsync<int>(async () =>
        {
            await Task.Delay(10);
    ...
    {
        var op = await Operation.TryAsync(Task.FromResult(123));
    ...
    {
        var failingTask = Task.FromException<int>(new Exception("task fail"));

        var op = await Operation.TryAsync(failingTask);
